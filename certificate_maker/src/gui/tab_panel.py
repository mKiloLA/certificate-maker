...
class TabPanel(tk.Frame):
        ...
        self.__build_file_input_tab(
            ...
            [
                (
                    "state_json",
                    "state_json",
                    "Browse for Certificate JSON file",
                    "JSON file",
                ),
                ...
                (
                    "zoom_survey",
                    "zoom_survey",
                    "Browse for Zoom Survey file",
                    "Zoom Survey file",
                ),
                (
                    "state_attendance",
                    "state_attendance",
                    "Browse for Attendance Sheet",
    ...
    def __build_file_input_tab(
        ...
        for row, (label_key, action, button_text, label_text) in enumerate(fields):
            ...
            self.__add_file_selector(
                tab, row, label_key, action, button_text, label_text
            )
    ...
    def action_performed(self, text: str) -> None:
        ...
        if text == "zoom":
            ...
            self.__file_labels["create_zoom"].configure(
                text="{} selected.".format(filename)
            )
            self.__file_labels["verify_zoom"].configure(
                text="{} selected.".format(filename)
            )
            ...
            self.__file_labels["create_webinar"].configure(
                text="{} selected.".format(filename)
            )
            self.__file_labels["verify_webinar"].configure(
                text="{} selected.".format(filename)
            )
        ...
        # Processing actions validate inputs before calling the data layer.
            ...
            if self.__zoom_file is not None and self.__webinar_file is not None:
                try:
                    # Create certificates
                    self.terminal.print_message("Certificate creation started . . .")
                    self.__json_file = create_certificates(
                        self.__zoom_file, self.__webinar_file
                    )
                    self.terminal.print_message(". . . Certificate creation finished!")

                    # Automatically select the json file
                    if self.__json_file:
                        filename = self.__json_file.split("/")[-1]
                        self.__file_labels["email_json"].configure(
                            text="{} selected.".format(filename)
                        )
                        self.__file_labels["state_json"].configure(
                            text="{} selected.".format(filename)
                        )
                        self.terminal.print_message(f"JSON file: {filename} selected.")
                    else:
                        self.terminal.print_message(
                            "Certificate creation failed: No JSON file was created."
                        )
                except IncorrectDateTimeFormat as e:
                    self.terminal.print_message(
                        f"Check time format: Failed to parse time information for `{e}`."
                    )
                except IncorrectNumberOfBreaks as e:
                    self.terminal.print_message(
                        f"Check Breaks: Zoom file asks for `{e}` breaks, but not enough times were given."
                    )
                except IncorrectBreakDate:
                    self.terminal.print_message(
                        "Check Breaks: The date of the breaks does not match the date of the CLE."
                    )
                except IncorrectWebinarTitle as e:
                    self.terminal.print_message(
                        f"Check CLE name: There are no CLEs matching `{e}`."
                    )
                except MasterListMissingHours as e:
                    self.terminal.print_message(
                        f"Check Master CLE List: CLE list is missing total hours in `{e}`."
                    )
                    ...
                    for entry in e:
                        name = "".join(
                            [x for x in entry if x not in ["(", ")", "'", ","]]
                        )
                        temp.append(name)
                    e = temp
                    self.terminal.print_message(
                        f"Check State Approvals: `{e[0]}` has no approval infomation in the state of `{e[1]}`."
                    )
                except AttorneyMissingState as e:
                    self.terminal.print_message(
                        f"Check State: `{e}` has no state listed."
                    )
                except AttorneyInvalidState as e:
                    self.terminal.print_message(
                        f"Check State: `{e}` does not have a valid state listed."
                    )
                except AttorneyMissingBarNumber as e:
                    self.terminal.print_message(
                        f"Check Bar Number: `{e}` has no bar number listed."
                    )
                except AttorneyInvalidBarNumber as e:
                    self.terminal.print_message(
                        f"Check Bar Number: `{e}` does not have a valid bar number listed."
                    )
                except MissingBreakRow:
                    self.terminal.print_message(
                        "Check Zoom File: There is no `Breaks` row in the CSV file."
                    )
                except MissingStartRow:
                    self.terminal.print_message(
                        "Check Zoom File: There is no `Start` row in the CSV file."
                    )
                except MismatchingStateAndBarNumbers as e:
                    self.terminal.print_message(
                        f"Check Bar Number and State: `{e}` has a mismatching number of bar numbers and states."
                    )
            ...
            try:
                ...
                self.terminal.print_message(
                    f"Unknown Error: Email the files and the following error message to Zak so he can add error checks for it in the future: `{e}`"
                )
            ...
            self.__json_file = self.__select_file(
                "state_json", "Browse for Certificate JSON file"
            )
        elif text == "poll":
            ...
            if all(file_path for file_path, _ in required_files):
                ...
                try:
                    self.terminal.print_message(
                        "State submission report generation started . . ."
                    )
                    ...
                    self.terminal.print_message(
                        f"State Submission File Missing: `{e.args[0]}` was not found."
                    )
                except StateSubmissionInvalidJson as e:
                    self.terminal.print_message(
                        f"State Submission JSON Error: `{e.args[0]}` is not valid certificate JSON."
                    )
            ...
            if self.__zoom_file is not None and self.__webinar_file is not None:
                try:
                    ...
                    create_certificates(
                        self.__zoom_file, self.__webinar_file, create=False
                    )
                    self.terminal.print_message(
            ...
            if not self.__attend_file:
                return
            self.terminal.print_message(
                f"Submission file: {self.__attend_file} selected."
            )
            ...
            self.__on_demand_file = self.browse_for_file(
                "Browse for on-demand-reference.xlsx file"
            )
            if not self.__on_demand_file:
                return
            filename = self.__on_demand_file.split("/")[-1]
            self.__file_labels["on_demand"].configure(
                text="{} selected.".format(filename)
            )
            self.terminal.print_message(
                f"On-Demand Reference file: {filename} selected."
            )
        elif text == "submit-on-demand":
            if (
                self.__attend_file is not None
                and self.__eval_file is not None
                and self.__on_demand_file is not None
            ):
                try:
                    self.terminal.print_message(
                        "On-Demand Report generation started . . ."
                    )
                    create_on_demand_report(
                        self.__attend_file, self.__eval_file, self.__on_demand_file
                    )
                    self.terminal.print_message(
                        ". . . On-Demand Report generation finished!"
                    )
                except MalformedEvaluationQuestionResponse as e:
                    self.terminal.print_message(
                        f"Check Evaluation Question Responses: `{e}` has a malformed response to an evaluation question."
                    )
                except MalformedCROString as e:
                    self.terminal.print_message(
                        f"Check Submitted CRO cell on Submission Report: `{e}` has a CRO string that is not in the format `State: BarNumber - Hours`."
                    )
                except IncorrectDateTimeFormat as e:
                    self.terminal.print_message(
                        f"Check time format: Failed to parse time information for `{e}`."
                    ...
                    self.terminal.print_message(
                        f"Check On-Demand Reference File: There is no sheet named `{e.args[0]}` in the reference file."
                    )
                except MissingSubmissionData as e:
                    self.terminal.print_message(
                        f"Submission Report missing data: `{e.args[0]}` is missing required data for course `{e.args[1]}`."
                    )
                except MissingEvaluationData as e:
                    self.terminal.print_message(
                        f"Evaluation Report missing data: `{e.args[0]}` is missing required data for course `{e.args[1]}`."
                    )
                except ReferenceFileMissingCourse as e:
                    self.terminal.print_message(
                        f"Check On-Demand Reference File: There is no course titled `{e.args[0]}` in the reference file for the state of `{e.args[1]}`."
                    )
                except Exception as e:
                    self.terminal.print_message(
                        f"Unknown Error: Email the files and the following error message to Zak so he can add error checks for it in the future: `{e}`"
